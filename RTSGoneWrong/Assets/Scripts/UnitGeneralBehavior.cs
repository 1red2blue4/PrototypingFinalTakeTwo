...
public class UnitGeneralBehavior : MonoBehaviour {
    ...
    [SerializeField] private GameObject attackingEnemy;
    ...
    void CircleDetectionForAttack()
    {
        ...
        if (hitColliders.GetLength(0) > 0)
        {
            ...
            if (enemyColliders.Count > 0)
            {
                ...
                {
                    if ((myUnitScript.thisUnitType == UnitScript.unitTypes.Healer && !(collider.gameObject.GetComponent<UnitScript>().isDamaged)) || (collider.gameObject == gameObject))
                    {
                        continue;
                    }
